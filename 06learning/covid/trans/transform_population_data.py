...
df_raw_population = spark.read.csv("/mnt/covidreportingdevelopdl/raw/population", sep=r'\t', header=True)
...
df_dim_country = spark.read.csv("/mnt/covidreportingdevelopdl/lookup/dim_country", sep=r',', header=True)
...
df_processed_population.write.format("com.databricks.spark.csv").option("header","true").option("delimiter", ",").mode("overwrite").save("/mnt/covidreportingdevelopdl/processed/population")
